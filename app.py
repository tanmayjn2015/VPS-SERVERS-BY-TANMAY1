...
NORMAL_PASSWORD = os.environ.get("NORMAL_PASSWORD", "2015")
...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if settings.get("maintenance"):
            return render_template("maintenance.html", message=settings.get("maintenance_msg", "Under maintenance"), site_name=settings.get("site_name", "TAHMID CODEX VPS"), theme_color=get_theme_color())
...
def login():
    ...
    if request.method == "POST":
        ...
        if password != normal_pass:
            return render_template("login.html", error="Wrong password", theme_color=get_theme_color(), site_name=settings.get("site_name", "TANMAY  VPS"))
    ...
    data = load_data()
    settings = data.get("settings", {})
    return render_template("login.html", error=None, theme_color=get_theme_color(), site_name=settings.get("site_name", "TAHMID CODEX VPS"))
...
def dashboard():
    ...
    data = load_data()
    settings = data.get("settings", {})
    site_name = settings.get("site_name", "TAHMID CODEX VPS")
